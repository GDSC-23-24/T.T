...
import { View, Text, StyleSheet, ScrollView, TextInput, ImageBackground, Image, TouchableOpacity } from 'react-native';
...
const FishBowlCommunity = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const communityResponse = await fetch('http://10.0.2.2:8080/api/fish-bowl/community', {
                headers: {
            ...
            const likesResponse = await fetch('http://10.0.2.2:8080/api/fish-bowl/likes', {
                headers: {
                    Accept: 'application/json',
                    'Content-Type': 'application/json',
                    Authorization: 'Bearer ' + API_TOKEN,
    ...
    return (
        <ImageBackground source={require('../../Asset/img/background_bowl.png')} style={styles.backgroundImage}>
            <View style={styles.container}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.scrollView}>
                    {trashDtos.map((trash) => (
                        <View key={trash.id} style={styles.trashContainer}>
                            <Image source={{ uri: trash.trashImageUrl }} style={styles.trashImage} />
                            <Text style={styles.createdAtText}>{trash.createdAt}</Text>
                            <View style={styles.userInfoContainer}>
                                <Image source={{ uri: trash.member.profileImageUrl }} style={styles.profileImage} />
                                <Text style={styles.nicknameText}>{trash.member.nickname}</Text>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.likesScrollView}>
                    {likesData.map((like) => (
                        <View key={like.id} style={styles.likesContainer}>
                            <Image
                                ...
                                />
                            <Text style={styles.likesNicknameText}>{like.memberDto.nickname}</Text>
                            <Text style={styles.likesCountText}>{like.likesCount} Likes</Text>
